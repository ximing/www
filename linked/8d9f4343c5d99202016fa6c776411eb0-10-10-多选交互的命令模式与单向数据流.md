...
- `selection` 使用 Set。选中判断是高频操作，Set 的 `has` 是 O(1)，数组的 `indexOf` 是 O(n)。同一个 id 不会重复出现，Set 会自动去重。
...
早期实现把 reducer 的逻辑分散在 DOM 事件监听器中，交互仍可运行，但状态规则难以集中检查。
...
事件数量增加后，无法从单一位置确认一次操作后的 `selection` 状态，调试需要检查多个回调。同一逻辑也会重复出现：Ctrl+click 的切换逻辑在 click 回调里写了一遍，万一以后加一个右键菜单的「切换选中」又得再写一遍。
...
这种分层会增加实现成本。只有一两种交互时，不必引入 reducer。文件列表同时包含单击、多选、连续选择、全选、拖框和键盘移动，且列表本体、底栏按钮和右键菜单都要读取 `selection`，因此适合集中维护状态。

## 实现中的后续改进

这套实现已在云盘 Web 端运行近三个月。后续可以改进以下方面：一是 reducer 里 `handleItemClick` 的几个分支可以再抽成独立的小函数，switch 那段会更干净；二是键盘移动的焦点逻辑应该和选中逻辑写成两个 reducer 组合而不是揉在一起，当初赶时间没拆；三是 rangeSelect 遇到过滤/排序后的列表时，区间计算必须用当前展示的 items 顺序而不是原始数据顺序，这一点最早漏了，后来补的。

新增交互时，只需增加 command 分支和适配层事件，无需在多个 DOM 回调中同步修改选择状态。后续的[虚拟滚动](./10-24-文件管理器虚拟滚动从零实现列表与缩略图双模式.md)和[文件选择交互](./10-17-云盘文件选择交互实现.md)继续复用这条状态路径。
